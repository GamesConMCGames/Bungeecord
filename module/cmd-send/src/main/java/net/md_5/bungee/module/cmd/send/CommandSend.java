...
public class CommandSend extends Command implements TabExecutor
{
    ...
    public void execute(CommandSender sender, String[] args)
    {
        if ( args.length != 2 )
        {
            sender.sendMessage( ChatColor.GREEN + "Not enough arguments, usage: /send <server|player|all|current> <target>" );
            return;
        }
        ...
        } else
        {
            ...
            } else
            {
                ...
                if ( player == null )
                {
                    sender.sendMessage( ChatColor.GREEN + "That player is not online" );
                    return;
                }
                summon( player, target, sender );
            }
        }
        sender.sendMessage( ChatColor.BLUE + "Successfully summoned player(s)" );
    }
    ...
    private void summon(ProxiedPlayer player, ServerInfo target, CommandSender sender)
    {
        if ( player.getServer() != null && !player.getServer().getInfo().equals( target ) )
        {
            ...
            player.sendMessage( ChatColor.BLUE + "Summoned to " + target.getName() + " by " + sender.getName() );
        }
    }
